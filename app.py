...
class FF_CLIENT(threading.Thread):
    ...
    # ---------------- MỚI: HÀM JOIN ONLY (Chỉ vào phòng) ----------------
    def api_join_only(self, team_code):
        # Nếu bot đang chạy chức năng khác thì dừng lại
        if self.auto_start_running:
            self.stop_auto = True
            self.auto_start_running = False
            time.sleep(1) # Chờ 1 chút cho vòng lặp cũ dừng hẳn
            
        self.stop_auto = False # Reset cờ stop
        
        if self.socket_client is None:
            return "Lỗi: Socket chưa kết nối (Online socket not ready)."

        try:
            log_message(f"Đang vào phòng {team_code} (Join Only Mode)...")
            join_teamcode(self.socket_client, team_code, self.key, self.iv)
            return f"Đã gửi lệnh Join vào team {team_code}. Bot sẽ ở yên trong phòng."
        except Exception as e:
            log_message(f"Lỗi Join: {e}")
            return f"Lỗi khi join: {e}"

    # ---------------- MỚI: VÒNG LẶP LV (Start mỗi 5s - Không Out) ----------------
    def lv_loop(self):
        log_message("--- BẮT ĐẦU CHẾ ĐỘ /lv (Start mỗi 5s - Không Out) ---")
        
        while not self.stop_auto:
            try:
                if self.socket_client is None:
                    log_message("Mất kết nối socket, đợi 5s...")
                    time.sleep(5)
                    continue

                # Tạo và gửi gói tin Start
                start_packet = self.start_autooo()
                if start_packet:
                    self.socket_client.send(start_packet)
                    log_message("[LV Mode] Đã bấm Start.")
                else:
                    log_message("[LV Mode] Lỗi tạo gói tin Start.")

                # Đợi 5 giây (kiểm tra stop mỗi 1 giây để dừng cho nhanh)
                for _ in range(5):
                    if self.stop_auto: break
                    time.sleep(1)
                    
            except Exception as e:
                log_message(f"Lỗi vòng lặp LV: {e}")
                time.sleep(2)
        
        log_message("--- ĐÃ DỪNG CHẾ ĐỘ /lv ---")

    def api_start_lv_mode(self):
        if self.auto_start_running:
            return "Bot đang chạy chức năng khác. Hãy dùng /stop trước."
        
        self.auto_start_running = True
        self.stop_auto = False
        
        # Chạy vòng lặp LV trong luồng riêng
        t = threading.Thread(target=self.lv_loop, daemon=True)
        t.start()
        
        msg = "Đã bật chế độ LV: Start mỗi 5s (Không out)."
        log_message(msg)
        return msg
...
def api_help():
    help_text = (
        ...
        "<b>Commands:</b><br>"
        "1. <a href='/start/123456'>/start/{team_code}</a> - Start/Out Auto Loop<br>"
        "2. <a href='/join/123456'>/join/{team_code}</a> - Join Only (No Start/Out)<br>"
        "3. <a href='/lv'>/lv</a> - Start every 5s (No Out)<br>"
        "4. <a href='/stop'>/stop</a> - Stop bot<br>"
        "5. <a href='/help'>/help</a> - Show this page<br><br>"
        "<b>Live Logs:</b><br>"
    )
...
def api_join_route(team_code):
    ...
    # Gọi hàm join only
    msg = bot_instance.api_join_only(team_code)
    
    return jsonify({
        "status": "success",
        "command": f"/join/{team_code}",
        "message": msg,
        "logs": log_buffer[-5:]
    })
...
def api_lv_route():
    ...
    # Gọi hàm chạy chế độ lv
    msg = bot_instance.api_start_lv_mode()
